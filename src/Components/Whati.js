...
function Whati() {
  ...
  useEffect(()=>{
    axios.get('http://localhost:8000/api/front/features')
      .then(res => {
        setData(res.data)
      })
      .catch(err => {
                ...
                <div className='categories-container row'>
                  
                  {
                    data.data.map((feature, index) =>{
                      return (
                        <div key={'feature'+index} data-aos="fade-up" data-aos-delay={index * 100} className='what-con col-sm-12 col-md-6 col-lg-6 col-xl-4 my-lg-4'>
