    ...
    const handleSubmit = (e) => {
        e.preventDefault();
            ...
            validator.validateAll(formData)
              .then(isValid => {
                if(isValid){
                    axios.post('http://localhost:8000/api/sendmail', formData)
                    .then(res => {
                    ...
                    <form className="contact-form row" onSubmit={handleSubmit}>
                        <div className="col-lg-12">
                            <div className="form-group">
                                ...
                                {
                                    errors.has('name') && (
                                    <div className='text-danger  mt-2'>{errors.first('name')}</div>
                                    )
                                }
                            </div>
                        </div>
                          <div className="col-lg-12">
                            <div className="form-group">
                                  ...
                                  {
                                    errors.has('email') && (
                                    <div className='text-danger  mt-2'>{errors.first('email')}</div>
                                    )
                                  }
                              </div>
                          </div>
                          <div className="col-lg-12">
                            <div className="form-group">
                                ...
                                {
                                    errors.has('msg') && (
                                    <div className='text-danger  mt-2'>{errors.first('msg')}</div>
                                    )
                                  }
                            </div>
                          </div>
                          <div className='contact-butt-con'>
                              <button type="submit" className='contact-butt hvr-sweep-to-right'>SEND MESSAGE<i className='fa-solid fa-arrow-right fa-lg ms-2'></i></button>
                          </div>
